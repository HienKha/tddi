...
- train CSV (`--train_path`) with a required `class` column
- validation CSV (`--valid_path`) with a required `class` column
- test CSV (`--test_path`) with a `class` column for evaluation metrics (optional for inference-only predictions)

Feature handling in preprocessing:

- categorical columns are inferred from `object`/`category` dtypes and label-encoded per column
- numeric columns are kept as continuous features
- the target label column name is fixed as `class`
